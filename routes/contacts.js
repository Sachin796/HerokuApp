...
router.post("/", auth, async (req, res) => {
  try {
    ...
    if (tempFirstName) {
      ...
      // if first name and last name is same then contact is already available
      if (lastName) {
    ...
    res
      .status(200)
      .json({ status: 0, message: "contact addedd", contact: contact1 });
  } catch (error) {
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (!contacts.length) {
      return res.status(200).json({ status: 0, message: "no contacts found!" });
    }

    res
      .status(200)
      .json({ status: 0, message: "contacts found", contact: contacts });
  } catch (error) {
    console.log(`contacts - ${error.message}`);
    res.status(500).json({ status: 1, message: "unexpected error occured!" });
  }
  ...
  try {
    ...
    if (bookmark === "true") {
      ...
      if (!contacts.length) {
        return res
          .status(200)
          .json({ status: 0, message: "no contacts found!" });
      }
...
router.put('/toggleBookmark',auth,async (req,res)=>{
  try{
    ...
    let bookmarkState = await Contact.find({ userID: id, _id:contactID }).select('isBookMark');
    ...
    await Contact.update({_id:contactID}, {$set: {isBookMark:toogleBookMark}}, function(err, doc) {
      if (err) res.status(500).json({ status: 1, message: "unexpected error occured!" });
      
      return res.status(200).json({ status: 0, message: "bookmark toggled" });
      });
...
router.put("/", auth, async (req, res) => {
  ...
  try {
    ...
    if(!foundContact){
      return res.status(200).json({status:1,message:"no contact found"})
    }
    ...
    return res
      .status(200)
      .json({ status: 0, message: "contact updated", contact: contact1 });
...
router.delete("/", auth, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({status:0,"message":"contact deleted"});
  } catch {
